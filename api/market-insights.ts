...
type GdeltDoc = {
  url?: string; title?: string; sourceCountry?: string;
  sourceCollection?: string; language?: string; seendate?: string;
};
...
// ─────────────────────────────────────────────────────────────────────────────
// BUILT-IN TEXTILE MARKET KNOWLEDGE FALLBACK
// Works with ZERO API keys — gives real, useful estimates.
// ─────────────────────────────────────────────────────────────────────────────

const TEXTILE_KNOWLEDGE: Record<string, {
  priceRange: string; priceUnit: string;
  demand: string; supply: string; trend: string; risk: string; hubs: string;
}> = {
  'cotton yarn': {
    priceRange: '₹220–₹320', priceUnit: 'per kg',
    demand: 'High year-round; peaks Oct–Feb (winter apparel season) and Apr–Jun (export season)',
    supply: 'Majority sourced from Coimbatore, Tiruppur and Erode spinning mills (Tamil Nadu) and Surat mills (Gujarat)',
    trend: 'Prices correlate with MSP of raw cotton (currently ₹6,620/quintal); softened ~5% from 2024 peak due to global oversupply',
    risk: 'Raw cotton crop shortfall (monsoon dependency) can spike prices 15–25% in 4–6 weeks',
    hubs: 'Tiruppur, Coimbatore, Erode (TN); Surat, Amreli (GJ)'
  },
  'polyester blend': {
    priceRange: '₹80–₹160', priceUnit: 'per kg',
    demand: 'Steady; driven by readymade garment exports and domestic fast-fashion',
    supply: 'Dominated by Reliance Industries (feedstock), Surat weavers, and Bhiwandi processors',
    trend: 'Prices track crude oil (PTA/MEG feedstock); stable at current oil prices (~$75/bbl)',
    risk: 'Any crude oil spike above $90/bbl typically lifts polyester prices by 8–12%',
    hubs: 'Surat (GJ), Bhiwandi (MH), Ludhiana (PB)'
  },
  'denim': {
    priceRange: '₹160–₹280', priceUnit: 'per meter',
    demand: 'Moderate; casualwear segment growing 6–8% YoY; export demand from EU and US stable',
    supply: 'Major mills in Ahmedabad (Arvind, Aarvee), Anand (GJ) and Kolkata (WB)',
    trend: 'Premium stretch-denim commands ₹240–₹280; basic denim softened ~3% from 2024',
    risk: 'Power cost increases and water scarcity in Ahmedabad affect bleaching / washing costs',
    hubs: 'Ahmedabad, Anand (GJ); Kolkata (WB)'
  },
  'silk': {
    priceRange: '₹2,500–₹6,000', priceUnit: 'per kg (raw reeled)',
    demand: 'Seasonal peak: Oct–Feb (wedding season); export demand to China & UAE',
    supply: 'Karnataka (Mysuru, Ramanagara) produces ~50% of India\'s raw silk; Assam (Muga, Eri)',
    trend: 'Mulberry silk prices rose ~8% in 2024 due to Chinese demand rebound',
    risk: 'Pebrine disease in silkworms, unseasonal rainfall in Karnataka affect cocoon yields',
  ...
  'linen': {
    ...
    supply: 'Imported flax from Belgium, France; value-added processing in Kolkata / Bengaluru',
    trend: 'Premium natural fibre demand driving prices up 5–8% annually; INR depreciation adds cost for importers',
    risk: 'Import dependency on Europe; disruptions (shipping delays, EUR/INR fluctuation) spike lead times',
    hubs: 'Kolkata (WB), Bengaluru (KA), Mumbai (MH)'
  },
  'wool': {
    priceRange: '₹400–₹900', priceUnit: 'per kg',
    demand: 'Seasonal; Oct–Mar is peak; hosiery and shawl segment growing',
...
const getStateTip = (state: string): string => {
  ...
  if (s.includes('tamil') || s === 'tn') return 'Tamil Nadu is a top textile hub — Tiruppur drives hosiery exports worth ₹35,000 Cr/year; Coimbatore leads yarn production.';
  ...
  if (s.includes('maharashtra') || s === 'mh') return 'Maharashtra hosts Mumbai\'s textile trading market and Bhiwandi, the largest powerloom hub in Asia.';
  ...
  if (msg.includes('supply') || msg.includes('availab') || msg.includes('stock')) {
    if (kb) {
      return `🏭 **${prod} — Supply Chain**\n\n` +
        `• ${kb.supply}\n` +
        `• Lead time: typically 3–7 days within state, 7–14 days cross-state\n` +
        `• **Risk:** ${kb.risk}\n` +
        `• ${stateTip}`;
    }
  }
  ...
  if (roleLabel === 'buyer' && (msg.includes('tip') || msg.includes('buy') || msg.includes('purchase') || msg.includes('advice'))) {
    if (kb) {
      return `🛒 **Buying Tips — ${prod}**\n\n` +
        `• **Best price window:** Buy when cotton harvest arrives (Oct–Dec) for lowest prices\n` +
        `• **Price range to target:** ${kb.priceRange} ${kb.priceUnit}\n` +
        `• **Preferred hubs:** ${kb.hubs}\n` +
        `• **Negotiate on:** Payment terms (7–30 day credit), bulk discount >500 kg, packaging\n` +
        `• **Risk to watch:** ${kb.risk}`;
    }
  }
  ...
  if (roleLabel === 'seller' && (msg.includes('tip') || msg.includes('sell') || msg.includes('advice'))) {
    if (kb) {
      return `💼 **Selling Strategy — ${prod}**\n\n` +
        `• **Peak selling season:** ${kb.demand.split(';')[0]}\n` +
        `• **Target price:** ${kb.priceRange} ${kb.priceUnit}\n` +
        `• **Key buyers:** Export houses, garment manufacturers, online B2B (IndiaMART, TradeIndia)\n` +
...
const buildOfflineInsights = (product: string, state: string, userRole: string): MarketInsight[] => {
  ...
  if (!kb) {
    return [
      ...
      { type: 'demand', title: 'Market Demand', description: 'Indian textile demand growing at 6–8% CAGR; export target $100B by 2030. Tiruppur, Surat and Bhiwandi are key hubs.', confidence: 75, impact: 'high' },
      { type: 'supply', title: 'Supply Chain', description: 'Lead times: 3–7 days within state, 7–14 days cross-state. Cluster sourcing from TN, GJ, MH reduces cost and lead time.', confidence: 70, impact: 'medium' },
      { type: 'price', title: 'Pricing Guidance', description: 'Verify current rates through State Textile Corporation, IndiaMART, or local mandi. Prices vary by grade, season, and quantity.', confidence: 60, impact: 'high' }
    ];
  }
  ...
  return [
    ...
    },
    {
      type: 'supply',
      title: 'Supply Chain',
      description: `${kb.supply}. Key hubs: ${kb.hubs}. ${stateTip}`,
      confidence: 75,
      impact: 'medium'
    },
    {
...
// ─────────────────────────────────────────────────────────────────────────────
// LLM PROVIDERS  (Groq first — free; Gemini — fallback)
// ─────────────────────────────────────────────────────────────────────────────
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if ((res.status === 429 || errMsg.toLowerCase().includes('quota')) && attempt < 2) {
            await sleep(4000 * Math.pow(2, attempt)); continue;
          }
...
/** Try AI providers; if both absent/fail → returns null (caller uses offline fallback) */
const tryGenerate = async (prompt: string): Promise<string | null> => {
  const g = await tryGroq(prompt);
  if (g) return g;
  const m = await tryGemini(prompt);
  if (m) return m;
  return null;
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const district: string = filters?.district || 'All';
  const locationContext =
    `Country: ${filters?.country || 'India'}\n` +
    `State: ${state !== 'All' ? state : 'Across India'}\n` +
    `District: ${district !== 'All' ? district : 'Major textile hubs'}`;
  ...
  if (mode === 'chat') {
    if (hasAI) {
      ...
      try {
        ...
        if (isRateLimitError(e)) {
          res.status(503).json({ error: 'AI service temporarily busy. Please try again in a moment.', rateLimited: true, meta });
          return;
        }
      }
    }
    ...
    res.status(200).json({ text, meta, offline: !hasAI });
    return;
  }
  ...
  if (hasAI) {
    const prompt =
      ...
      `If no live data, label estimates clearly.`;

    try {
      const text = await tryGenerate(prompt);
      if (text) {
        const parsed = extractJsonArray(text);
        res.status(200).json(parsed ? { insights: parsed, meta } : { insights: [], raw: text, meta });
        return;
      }
    } catch (e: any) {
      if (isRateLimitError(e)) {
  ...
  res.status(200).json({ insights, meta, offline: !hasAI });
}
